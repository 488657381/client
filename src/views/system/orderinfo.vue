<template>
  ...
  <el-form ref="queryFormRef" :model="queryForm" :inline="true">
    ...
    <el-form-item label="下单日期" prop="gameName">
    ...
    <el-form-item label="订单状态" prop="orderStatus">
      <el-select
          ...
          style="width: 220px"
      >
        <el-option :value="1" label="下单"/>
        <el-option :value="2" label="接单"/>
  ...
  <el-table height="450" :data="tableData.tableList" style="width: 100%">
    <el-table-column prop="orderId" fixed label="订单编号" width="90" align="center"/>
    <el-table-column prop="orderDatetime" label="下单时间" width="120" align="center"/>
    <el-table-column prop="orderStatus" label="订单状态" width="120" align="center">
      <template #default="scope">
        <el-tag
            :type="scope.row.orderStatus === '1' ? 'primary' :
             scope.row.orderStatus === '2' ? 'success' :
             scope.row.orderStatus === '3' ? 'warning' : 'info'"

        >
          {{
            scope.row.orderStatus === '1' ? '下单' :
                scope.row.orderStatus === '2' ? '接单' :
                    scope.row.orderStatus === '3' ? '结单' : '评论'
          }}
        </el-tag>
      </template>
    </el-table-column>

    <el-table-column prop="orderSkillId" label="技能编号" width="120" align="center"/>
    <el-table-column prop="gameName" label="游戏名称" width="120" align="center"/>
    <el-table-column prop="orderSkillNum" label="订单数量" width="120" align="center"/>
    <el-table-column prop="orderSkillPrice" label="订单单价" width="120" align="center"/>
    <el-table-column prop="orderPaymentCustomerName" label="下单用户" width="120" align="center">
      <template #default="scope">
        <el-tag type="primary">{{ scope.row.orderPaymentUid + '-' + scope.row.orderPaymentCustomerName }}</el-tag>
      </template>
    </el-table-column>
    <el-table-column prop="orderServeCustomerName" label="接单用户" width="120" align="center">
      <template #default="scope">
        <el-tag type="success">{{ scope.row.orderServeUid + '-' + scope.row.orderServeCustomerName }}</el-tag>
      </template>
    </el-table-column>
    <el-table-column prop="orderRemark" label="订单备注" width="120" align="center"/>
    <el-table-column fixed="right" label="操作" width="200" align="center">
      <template #default="scope">
        <el-button type="primary" plain @click="opencomment(scope.row)">订单评论</el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <el-drawer title="订单评论" size="45%" v-model="commentVisable" direction="btt">
    <el-table height="250" :data="commentFrom.commentList" style="width: 100%">
      <el-table-column prop="commentId" fixed label="评论编号" width="150" align="center"/>
      <el-table-column prop="commentDatetime" label="评论时间" width="250" align="center">
        <template #default="scope">
      ...
      <el-table-column prop="commentDetail" label="评论详细" width="250" align="center">
        <template #default="scope">
          <el-popover effect="light" trigger="hover" placement="top" width="auto">
            <template #default>
              <div>评论详细：{{ scope.row.commentDetail }}</div>
            </template>
            <template #reference>
      ...
      <el-table-column fixed="right" label="操作" width="280" align="center">
        <template #default="scope">
          <el-button type="primary" @click="DeleteComment(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </el-drawer>
...
const myQuery = async (params) => {
  ...
  let resp = await myGet("/order/listOrder", params)
...
/**查看评论功能 */
//打开评论框
const opencomment = async (currentRow) => {
  commentVisable.value = true
  saveOrderId.value = currentRow.orderId
  // console.log(currentRow);
  await updataData()

}
const saveOrderId = ref()
//更新表单数据
const updataData = async () => {
  let resp = await myPost("/order/listComment", {orderId: saveOrderId.value})
  commentFrom.commentList.splice(0, commentFrom.commentList.length, ...resp.data.returnData)
}
//打开评论框
const commentVisable = ref(false)
//储存评论信息
const commentFrom = reactive({
  commentList: []
})
//修改评论状态
const updateComment = async (row) => {
  ...
  ElMessageBox.confirm(
    ...
    let resp = await myPost("/order/updateComment", {commentStatus: row.commentStatus, commentId: row.commentId})
...
const DeleteComment = (row) => {
  ElMessageBox.confirm(
      ...
      .then(async () => {
        //发送请求
        let resp = await myPost("/order/deleteComment", {commentId: row.commentId})
        if (resp.data.code == 20010) {
          ElMessage.success(resp.data.msg)
          await updataData()
          //当前查询条件
